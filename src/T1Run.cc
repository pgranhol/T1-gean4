// Run class for T1 based on hadr03
...
void T1Run::SumTrack(G4double trackl)
{
  fTotalCount++;
...
void T1Run::CountNuclearChannel(G4String name, G4double Q)
{
...
void T1Run::Balance(G4double Pbal)
{ 
  fPbalance[0] += Pbal;
  //update min max   
...
void T1Run::CountGamma(G4int nGamma)
{ 
  fGammaCount++;
  fNbGamma[0] += nGamma;
  //update min max   
...
void T1Run::EndOfRun(G4bool print) 
{
